...
Elogio.modules.sidebarModule = function (modules) {
    ...
    function initModule() {
        object.onButton = $('#elogio-on');
        object.offButton = $('#elogio-off');
        ...
        object.onButton.on('click', switchOn);
        object.offButton.on('click', switchOff);
    }

    function addOrUpdateCard(imageObj, sidebar) {
     ...
     */
    self.startPageProcessing = function (context, nodes, sidebar) {
        initModule();
        nodes = nodes || null;
        locator.findImages(context, nodes, function (imageObj) {
            addOrUpdateCard(imageObj, sidebar);
        }, function () {
            //on error
        }, function () {
            //on finished
